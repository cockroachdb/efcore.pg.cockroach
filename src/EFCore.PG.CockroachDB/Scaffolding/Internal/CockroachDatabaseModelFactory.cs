...
using Npgsql.EntityFrameworkCore.CockroachDB.Utilities;
...
public class CockroachDatabaseModelFactory : NpgsqlDatabaseModelFactory
{
    ...
    public override DatabaseModel Create(DbConnection dbConnection, DatabaseModelFactoryOptions options)
    {
        ...
        try
        {
            var internalSchemas = "'pg_catalog', 'information_schema', 'crdb_internal'";
...
LEFT JOIN pg_depend AS dep ON dep.refobjid = cls.oid AND dep.refobjsubid = attr.attnum AND dep.deptype IN ('a', 'i')
...
ORDER BY attnum";
        ...
        foreach (var tableGroup in tableGroups)
        {
            ...
            foreach (var record in tableGroup)
            {
                ...
                //
                if (record.GetValueOrDefault<bool>("attishidden"))
                {
                    table.Columns.Add(null!);
                    continue;
                }
